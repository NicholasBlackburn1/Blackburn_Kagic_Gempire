...
import mod.kagic.entity.ai.EntityAIGemFlyToTarget;
import mod.kagic.entity.ai.EntityAIGemRandomFlight;
import mod.kagic.entity.ai.EntityAITourmalineBlowAttack;
...
public class EntityCorruptedWatermelonTourmaline extends EntityCorruptedGem {
	...
	public EntityCorruptedWatermelonTourmaline(World world) {
		...
		this.dataManager.register(EntityCorruptedWatermelonTourmaline.ATTACKING, false);
		this.dataManager.register(EntityCorruptedWatermelonTourmaline.MOUTH_OPEN, 0);
	...
	public boolean isAttacking() {
		return this.dataManager.get(EntityCorruptedWatermelonTourmaline.ATTACKING);
	}
	...
	public void setAttacking(boolean attacking) {
		...
		this.dataManager.set(EntityCorruptedWatermelonTourmaline.ATTACKING, attacking);
	}
	
	public void setMouthOpen(int time) {
		this.dataManager.set(EntityCorruptedWatermelonTourmaline.MOUTH_OPEN, time);
	}
	
	public int getMouthOpen() {
		return this.dataManager.get(EntityCorruptedWatermelonTourmaline.MOUTH_OPEN);
	}
	...
	static class TourmalineMoveHelper extends EntityMoveHelper {
		...
		public void onUpdateMoveHelper() {
			if (this.action == EntityMoveHelper.Action.MOVE_TO) {
				...
				// KAGIC.instance.chatInfoMessage("Cooldown
				// is " + this.courseChangeCooldown);
				if (this.courseChangeCooldown-- <= 0) {
					this.courseChangeCooldown += this.parentEntity.getRNG().nextInt(5) + 2;
					euclideanDistance = MathHelper.sqrt(euclideanDistance);
		...
		/**
		 * Checks if entity bounding box is not colliding
		 * with terrain
		 */
