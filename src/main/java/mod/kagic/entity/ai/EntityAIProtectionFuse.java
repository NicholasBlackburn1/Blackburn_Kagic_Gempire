...
public class EntityAIProtectionFuse<P extends EntityGem, F extends EntityFusionGem> extends EntityAIBase {
	...
	public boolean shouldExecute() {
		List<P> list = this.initiator.world.<P>getEntitiesWithinAABB(this.passive, this.initiator.getEntityBoundingBox().grow(this.size, this.size, this.size));
	...
	public void updateTask() {
		if (this.initiator.getDistanceSq(this.target) > this.target.width * 3) {
			...
			F fusion;
			try {
				fusion = this.result.getDeclaredConstructor(World.class).newInstance(this.initiator.world);
	...
	private boolean checkTarget(P target) {
		return target != null && target.isTamed() && !target.isFusion() && target.getHealth() > 0 && !target.isDead && target.getRevengeTarget() != null && target.getRevengeTarget() != this.initiator && !target.getRevengeTarget().isDead;
	}
}