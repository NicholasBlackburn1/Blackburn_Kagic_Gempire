...
public class LayerGarnetItem implements LayerRenderer<EntityGem> {
	protected final RenderGemBase<?> livingEntityRenderer;
	
	public LayerGarnetItem(RenderGemBase<?> livingEntityRenderer) {
	...
	private void renderHeldItem(EntityGem entity, ItemStack stack, ItemCameraTransforms.TransformType camera, EnumHandSide handSide) {
		if (!stack.isEmpty()) {
			...
			GlStateManager.translate((float)(lefty ? -.875 : 1) / 3.125F, 0.1F, -0.85F);
