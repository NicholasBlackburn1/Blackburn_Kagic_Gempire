...
public class WorldDataWarpPad extends WorldSavedData {
	...
	public WorldDataWarpPad() {
		super(WorldDataWarpPad.DATA_NAME);
	}
	...
	public static WorldDataWarpPad get(World world) {
		if (!world.isRemote) {
			...
			WorldDataWarpPad instance = (WorldDataWarpPad) storage.getOrLoadData(WorldDataWarpPad.class, WorldDataWarpPad.DATA_NAME);
			if (instance == null) {
				// KAGICTech.instance.chatInfoMessage("Data
				// on server was null");
				instance = new WorldDataWarpPad();
				storage.setData(WorldDataWarpPad.DATA_NAME, instance);
			}
			return instance;
		} else {
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		while (it.hasNext()) {
			...
			tc.setInteger("y", pair.getKey().getY());
