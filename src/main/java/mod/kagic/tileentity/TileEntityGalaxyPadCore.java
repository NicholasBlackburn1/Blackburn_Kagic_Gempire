...
import mod.kagic.networking.EntityTeleportMessage;
...
public class TileEntityGalaxyPadCore extends TileEntityWarpPadCore {
	...
	protected void setDirty() {
		...
		if (!this.world.isRemote) {
			WorldDataGalaxyPad.get(this.world).addGalaxyPadEntry(this.name, this.isPadValid, this.isClear, this.getLocation());
		}
	}
	...
	protected boolean validateStairs() {
		...
		if (state.getBlock() != Blocks.STONE_SLAB || state.getValue(BlockStoneSlab.VARIANT) != BlockStoneSlab.EnumType.QUARTZ || state.getValue(BlockSlab.HALF) != EnumBlockHalf.BOTTOM) {
			return false;
		}
	...
	public void beginWarp(GalaxyPadLocation destination) {
		...
		this.warpTicksLeft = TileEntityWarpPadCore.warpTicks;
	...
	public void WARP() {
		...
		while (it.hasNext()) {
			...
			if (entity instanceof EntityPlayerMP) {
				...
				// With thanks to quadraxis from the
		...
		this.warping = false;
		this.cooling = true;
		this.cooldownTicksLeft = TileEntityWarpPadCore.warpCooldownTicks;
		this.setDirty();
	}
}
