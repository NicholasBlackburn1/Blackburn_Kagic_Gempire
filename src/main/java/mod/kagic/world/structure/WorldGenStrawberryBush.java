			...
			for (int y = -3; y < height; ++y) {
				for (int z = -length; z < length; ++z) {
					...
					if (a + b + c <= 1) {
						...
						if (world.getBlockState(genPos).getBlock().isReplaceable(world, genPos) && world.getBiome(genPos) == ModBiomes.STRAWBERRYBATTLEFIELD && world.getBlockState(genPos).getMaterial() != Material.WATER && world.getBlockState(genPos.down()).getMaterial() != Material.WATER) {
							if (rand.nextInt(WorldGenStrawberryBush.STRAWBERRY_CHANCE_RECIPROCAL) == 0) {
								world.setBlockState(genPos, ModBlocks.GIANT_STRAWBERRY.getDefaultState(), 2);
							} else {
								world.setBlockState(genPos, WorldGenStrawberryBush.BUSH_LEAVES, 2);
							}
						}
					}
				}
