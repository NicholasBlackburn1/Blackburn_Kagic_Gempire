...
public class RuinStructure extends WorldGenerator {
	...
	protected boolean checkBiome(World world, BlockPos pos) {
		// If we haven't defined any biomes, we can generate
		// anywhere
	...
	protected boolean checkCorners(World world, BlockPos pos) {
		...
		if (!this.allowedBlocks.contains(world.getBlockState(corner1))) {
			return false;
		}
		if (!this.allowedBlocks.contains(world.getBlockState(corner2)) || Math.abs(corner2.getY() - corner1.getY()) > this.foundationDepth) {
			return false;
		}
		if (!this.allowedBlocks.contains(world.getBlockState(corner3)) || Math.abs(corner3.getY() - corner1.getY()) > this.foundationDepth) {
			return false;
		}
		if (!this.allowedBlocks.contains(world.getBlockState(corner4)) || Math.abs(corner4.getY() - corner1.getY()) > this.foundationDepth) {
			return false;
		}
		return true;
	}
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if (!this.checkBiome(world, pos)) {
			// KAGIC.instance.chatInfoMessage("Biome check
			// failed");
			return false;
		}

		if (!this.checkDistance(world, pos)) {
			// KAGIC.instance.chatInfoMessage("Distance
			// check failed");
			return false;
		}

		if (!this.checkCorners(world, pos)) {
			// KAGIC.instance.chatInfoMessage("Corner check
			// failed");
			return false;
		}
		...
		if (!this.checkChunks(world, affectedChunks)) {
			// KAGIC.instance.chatInfoMessage("Existing ruin
			// check failed");
			return false;
		}
