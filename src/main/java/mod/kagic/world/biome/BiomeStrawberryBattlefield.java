...
import mod.kagic.world.structure.WorldGenFloatingIslands;
import mod.kagic.world.structure.WorldGenStrawberryBush;
...
public class BiomeStrawberryBattlefield extends Biome {
	...
	static {
		BiomeStrawberryBattlefield.PROPERTIES.setBaseHeight(0.4F);
		BiomeStrawberryBattlefield.PROPERTIES.setHeightVariation(0.05F);
		BiomeStrawberryBattlefield.PROPERTIES.setTemperature(0.9F);
		BiomeStrawberryBattlefield.PROPERTIES.setRainfall(0.5F);
		BiomeStrawberryBattlefield.PROPERTIES.setWaterColor(0xEAFFFC);
	}
	...
	class Decorator extends BiomeDecorator {
		...
		protected void genDecorations(Biome biome, World world, Random random) {
			...
			if (random.nextInt(Decorator.FLOATCHANCERECIPROCAL) == 0) {
				int offset = random.nextInt(Decorator.OFFSETVARIANCE) + (int) (16F + Decorator.VERTICALOFFSET);
