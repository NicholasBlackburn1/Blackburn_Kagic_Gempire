...
public class BiomeKindergarten extends Biome {
	...
	static {
		BiomeKindergarten.PROPERTIES.setBaseHeight(0.1F);
		BiomeKindergarten.PROPERTIES.setHeightVariation(0F);
		BiomeKindergarten.PROPERTIES.setRainDisabled();
		BiomeKindergarten.PROPERTIES.setWaterColor(0x646400);
	}

	public BiomeKindergarten() {
		this(BiomeKindergarten.PROPERTIES);
	}
	...
	public BiomeKindergarten(BiomeProperties properties) {
		...
		this.topBlock = Biome.AIR;// this.DRAINED_BLOCK;
		this.fillerBlock = Biome.AIR;// this.DRAINED_BLOCK;
	...
	public void genTerrainBlocks(World world, Random rand, ChunkPrimer chunkPrimer, int x, int z, double noiseVal) {
		...
		int i = (x & -16) + (z & 15);
		int j = (z & -16) + (x & 15);
		double d0 = Math.min(Math.abs(noiseVal), Math.abs(this.cliffNoise.getValue(i * 1D, j * 1D)));
		
		if (d0 > 0.0D) {
			// 0.001953125
			double d2 = Math.abs(this.cliffRoofNoise.getValue(i * 0.001953125D, j * 0.001953125D));
			peakHeight = d0 * d0 * 8D;
		...
		IBlockState drainedBlockState = BiomeKindergarten.DRAINED_BLOCK;

		for (int height = 255; height >= 0; --height) {
			if (height % 6 == 0 || height % 6 == 1) {
				drainedBlockState = BiomeKindergarten.DRAINED_BLOCK_2;
			} else if (height % 5 == 0) {
				drainedBlockState = BiomeKindergarten.DRAINED_BANDS;
			} else {
				drainedBlockState = BiomeKindergarten.DRAINED_BLOCK;
			}
			
			if (chunkPrimer.getBlockState(chunkZ, height, chunkX).getMaterial() == Material.AIR && height < (int) peakHeight) {
				chunkPrimer.setBlockState(chunkZ, height, chunkX, Biome.STONE);
			}
			
			if (height <= rand.nextInt(5)) {
				chunkPrimer.setBlockState(chunkZ, height, chunkX, Biome.BEDROCK);
			} else {
				if (chunkPrimer.getBlockState(chunkZ, height, chunkX).getBlock() == Blocks.STONE || chunkPrimer.getBlockState(chunkZ, height, chunkX).getBlock() == Biome.WATER) {
	...
	class Decorator extends BiomeDecorator {
		...
		private boolean canMakeHole(World world, BlockPos pos, EnumFacing direction) {
			for (int y = 0; y < Decorator.HOLE_HEIGHT + 1; ++y) {
				for (int z = 2; z < Decorator.HOLE_DEPTH + 2; ++z) {
					if (!world.isBlockFullCube(pos.up(y).offset(direction, z)) || !world.isBlockFullCube(pos.up(y).offset(direction, z).offset(direction.rotateY())) || !world.isBlockFullCube(pos.up(y).offset(direction, z).offset(direction.rotateYCCW()))) {
						return false;
					}
				}
			}
		...
		private void generateHole(World world, BlockPos pos, EnumFacing direction, Random rand) {
			if (rand.nextFloat() < Decorator.GEM_CHANCE) {
				...
				for (int z = 8; z < 24; z += 2) {
					...
					if (potentialHolePos.getY() <= 120) {
						...
						for (int y = 0; y < 60; y += 5) {
							EnumFacing dir = this.generateExitHole(world, potentialHolePos.up(y), random);
							if (dir != EnumFacing.DOWN) {
								holeDirection = dir;
							}
						}
						if (holeDirection != EnumFacing.DOWN && random.nextFloat() < Decorator.INJECTOR_CHANCE) {
							this.generateInjector(world, world.getTopSolidOrLiquidBlock(this.chunkPos.add(x, 0, z).up().offset(holeDirection, Decorator.HOLE_DEPTH)), holeDirection);
						}
					}
				}
			}
		...
		protected void generateOres(World worldIn, Random random) {
			// All minerals have been drained from this
			// biome
			return;
		}
	}
