...
public class Fogger {
	...
	public void onFogColor(EntityViewRenderEvent.FogColors event) {
		...
		if (biome == ModBiomes.FLOATINGPEAKS || biome == ModBiomes.KINDERGARTEN || this.fadeTicksLeft > 0) {
			...
			float f = 0.25F + 0.75F * Minecraft.getMinecraft().gameSettings.renderDistanceChunks / 32.0F;
			...
			if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(MobEffects.NIGHT_VISION)) {
				...
				float f15 = i > 200 ? 1.0F : 0.7F + MathHelper.sin((i - partialTicks) * (float) Math.PI * 0.2F) * 0.3F;
				float f6 = 1.0F / red;
	 ...
	 * onFogDensity(EntityViewRenderEvent.FogDensity event)
	 * { Entity entity = event.getEntity(); World world =
	 * entity.world; Biome biome =
	 * world.getBiome(entity.getPosition()); if (biome ==
	 * ModBiomes.KINDERGARTEN) { event.setDensity(0.5F);
	...
	public void onRenderBlockOverlay(RenderBlockOverlayEvent event) {
		if (event.getPlayer().world.getBlockState(event.getBlockPos()).getBlock() == ModBlocks.ROSE_TEARS) {
			Minecraft.getMinecraft().getTextureManager().bindTexture(Fogger.RES_UNDERTEARS_OVERLAY);
