...
public class OreDictListener {
	// The purpose of this class is to catch oredict entries
	// registered after we register the crux list
	// in ModEntities. This shouldn't be necessary, since
	// oredict registration *should* be done in
	// preInit, but some mods (such as Simple Gravel Ores)
	// do it in postInit for some reason.
	@SubscribeEvent
	public void onOreRegister(OreRegisterEvent event) {
		ItemStack ore = event.getOre();
		String name = event.getName();

		KAGIC.logger.warn("Note! The mod " + ore.getItem().getRegistryName().getResourceDomain() + " is registering the ore " + name + " to the ore dictionary after preInit. This is a violation of best practices and should be reported to the mod author.");
