...
public class BlockInjector extends Block {
	...
	public BlockInjector(boolean isEquipped, boolean analog) {
		...
		this.analog = analog;
		this.setDefaultState(this.blockState.getBaseState().withProperty(BlockInjector.FACING, EnumFacing.NORTH));
		if (this.isEquipped) {
	...
	public void injectGemSeed(World worldIn, BlockPos pos) {
		if (!(worldIn.getBlockState(pos.up()).getBlock() instanceof BlockIncubator)) {
			...
			if (newPosY > 5) {
				...
				worldIn.setBlockState(pos, ModBlocks.INJECTOR.getDefaultState().withProperty(BlockInjector.FACING, worldIn.getBlockState(pos).getValue(BlockInjector.FACING)));
	...
	public IBlockState onBlockPlaced(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
		return this.getDefaultState().withProperty(BlockInjector.FACING, placer.getHorizontalFacing().getOpposite());
	}
	
	@Override
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
		worldIn.setBlockState(pos, state.withProperty(BlockInjector.FACING, placer.getHorizontalFacing().getOpposite()), 2);
		...
		if (!worldIn.isRemote) {
			...
			if (playerIn.isSneaking()) {
				if (this.analog) {
					...
					if (!this.isEquipped) {
						worldIn.setBlockState(pos, ModBlocks.ANALOG_INJECTOR.getDefaultState().withProperty(BlockInjector.FACING, worldIn.getBlockState(pos).getValue(BlockInjector.FACING)));
					}
				}
			} else {
	...
	public int calcDistanceAutomatically(World worldIn, BlockPos pos) {
		...
		for (bestDepth = pos.down(6).getY(); bestDepth > worstDepth; --bestDepth) {
			...
			if (worldIn.getBlockState(newBlockPos).getBlock() == ModBlocks.GEM_SEED || worldIn.getBlockState(newBlockPos).getBlock() == Blocks.BEDROCK || !worldIn.isSideSolid(newBlockPos, EnumFacing.UP) || !worldIn.isSideSolid(newBlockPos.up(), EnumFacing.DOWN)) {
				bestDepth -= 3;
			} else {
	...
	private void setDefaultFacing(World worldIn, BlockPos pos, IBlockState state) {
		if (!worldIn.isRemote) {
			...
			EnumFacing enumfacing = state.getValue(BlockInjector.FACING);
			...
			worldIn.setBlockState(pos, state.withProperty(BlockInjector.FACING, enumfacing), 2);
		}
	}
	public static void setState(boolean active, World worldIn, BlockPos pos) {
		IBlockState iblockstate = worldIn.getBlockState(pos);
		worldIn.setBlockState(pos, ModBlocks.INJECTOR.getDefaultState().withProperty(BlockInjector.FACING, iblockstate.getValue(BlockInjector.FACING)), 3);
		worldIn.setBlockState(pos, ModBlocks.INJECTOR.getDefaultState().withProperty(BlockInjector.FACING, iblockstate.getValue(BlockInjector.FACING)), 3);
	}
	...
	public IBlockState getStateFromMeta(int meta) {
		...
		return this.getDefaultState().withProperty(BlockInjector.FACING, enumfacing);
	}
	
	@Override
	public int getMetaFromState(IBlockState state) {
		return state.getValue(BlockInjector.FACING).getIndex();
	}
	
	@Override
	public IBlockState withRotation(IBlockState state, Rotation rot) {
		return state.withProperty(BlockInjector.FACING, rot.rotate(state.getValue(BlockInjector.FACING)));
	}
	
	@Override
	public IBlockState withMirror(IBlockState state, Mirror mirrorIn) {
		return state.withRotation(mirrorIn.toRotation(state.getValue(BlockInjector.FACING)));
	}
	
	@Override
	protected BlockStateContainer createBlockState() {
		return new BlockStateContainer(this, new IProperty[]{BlockInjector.FACING});
	}
