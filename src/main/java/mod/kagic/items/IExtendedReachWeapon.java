...
public interface IExtendedReachWeapon {
	...
	class MouseClickHandler {
		...
		public void onMouseClick(MouseEvent event) {
			...
			KAGIC.logger.info("got player Wepon"+player.getHeldItemMainhand().getItem().getUnlocalizedName());

			if(player.getHeldItemMainhand().getItem().getUnlocalizedName().equals("item.obsidians_sword")){
				IExtendedReachWeapon wep2 = (IExtendedReachWeapon) player.getHeldItemMainhand().getItem();
			Optional<RayTraceResult> result = getMouseOverExtended(wep2.getReach());
			...
			result.ifPresent(r -> {
				if (r.entityHit != null && r.entityHit.hurtResistantTime == 0 && r.entityHit != player) {
					...
					KAGIC.logger.info("Sent Reach packet to server! yaya");
				}
			});
		}
